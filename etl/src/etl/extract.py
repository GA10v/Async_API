...
class PostgresExtracting:
    ...
    def _extract_films(self) -> Iterator[dict]:
        ...
        while films := self.cur.fetchmany(self.extract_size):
            yield from films
            self.state.set_state('films', str(films[-1].get('modified')))
    ...
    def _extract_films_persons(self) -> Iterator[DictRow]:
        ...
        if persons_id:
            ...
            while films := self.cur.fetchmany(self.extract_size):
                ...
                self.state.set_state('persons_film', str(films[-1].get('modified')))

            self.state.set_state('films_persons', str(persons[-1].get('modified')))
    ...
    def _extract_films_genres(self) -> Iterator[DictRow]:
        ...
        if genres_id:
            ...
            while films := self.cur.fetchmany(self.extract_size):
                ...
                self.state.set_state('genres_film', str(films[-1].get('modified')))

            self.state.set_state('films_genres', str(genres[-1].get('modified')))
    ...
    def _extract_persons(self) -> Iterator[DictRow]:
        ...
        while persons := self.cur.fetchmany(self.extract_size):
            ...
            self.state.set_state('persons', str(persons[-1].get('modified')))
    ...
    def _extract_genres(self) -> Iterator[DictRow]:
        ...
        while genres := self.cur.fetchmany(self.extract_size):
            ...
            self.state.set_state('genres', str(genres[-1].get('modified')))
